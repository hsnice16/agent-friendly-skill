...
- **Kimi CLI** weight profile — Moonshot AI's terminal coding agent is now the 9th scored model, so `modelScores` returns 9 entries and Kimi CLI can be picked as best-fit. `agents_md` weighted 1.0 (AGENTS.md is its only instruction surface; there is no CLAUDE.md fallback and `/init` generates one), `tests` 0.9, `deps_manifest` / `dev_env` / `readme` 0.7.
